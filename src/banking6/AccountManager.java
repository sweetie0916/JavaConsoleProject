...
//		}
	...
	public void readAccInfo() {
		try {
			...
			// 파일에 계좌정보가 몇개 저장되었는지 확인할 수 없으니 무한루프 구현
			while(true) {
				// 직렬화될 때는 Object기반으로 저장되므로 역직렬화할때는 반드시 다운캐스팅한다.
				Account ac = (Account) in.readObject();
				
				// HashSet<Account> 컬렉션에 추가
				accountArr.add(ac);
			}
		}
		catch(Exception e) {
	...
	public void autoSave(AutoSaver auto){
		...
		if(keyInput==1) { // 자동저장 On -> start()
			/* isAlive() : 해당 쓰레드의 run()메서드가 종료됐는지에 따라
						   살았는지 죽었는지 확인할때 사용
			*/
